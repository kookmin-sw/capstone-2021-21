...
class Searchpage:
    ...
    def __parse_searchdata(self):
        ...
        for i in self.get_items:
            ...
            #JSON으로 아이템 저장하기
            self.json_result.append({"id":index, "bookname" : title, "description" : description, "imgurl" : imgurl , "mall" : list(instock_shop.keys())})
            index += 1
...
class Search_result(Searchpage):
    ...
    def __parse_itemdata(self):
        ...
        for i in range(0, len(self.title)):
            ...
            stores = ", ".join(str(data) for data in self.target_shops[i].keys()) #이건 검색결과에서 매장 어디어디 있는가 보여주는 것들
            shops_stock = []
            ...
            self.result.append({'search_id' : i, 'bookname' : self.title[i], 'description': self.description[i], 'stores': stores, 'imgurl' : self.img_urls[i], 'result' : shops_stock})
